...
import { TFieldsListToolbarProps } from './types';

const FieldsListToolbar: React.FC<TFieldsListToolbarProps> = ({ isRowsSelected = false }) => {
  const handlers = useHandlers();
  const { dataView } = useContext(Store);
  const data = dataView.state.data['fields'];
...
export default FieldsListToolbar;
