...
import { TLessonsListToolbarProps } from './types';

const LessonsListToolbar: React.FC<TLessonsListToolbarProps> = ({ isRowsSelected = false }) => {
  const handlers = useHandlers();
  const { dataView } = useContext(Context);
  const data = dataView.state.data['training-lessons-list'];
...
export default LessonsListToolbar;
