...
import { TQuestionsListToolbarProps } from './types';

const QuestionsListToolbar: React.FC<TQuestionsListToolbarProps> = ({ isRowsSelected = false }) => {
  const handlers = useHandlers();
  const { dataView } = useContext(Context);
  const data = dataView.state.data['training-questions-list'];
...
export default QuestionsListToolbar;
